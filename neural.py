### RNN Solution to Jeopardy Category Prediction
### rnn.py
...
class Decoder(torch.nn.Module):
    ...
    def step(self, fencs, h, enum):
        ...
        cross_att = torch.log_softmax(bmv(fencs, a), dim=-1)
        return (o, h, cross_att)
...
class Model(torch.nn.Module):
    ...
    def logprob(self, fwords, ewords):
        ...
        for i in range(1, len(ewords)):
            o, h, cross_att = self.dec.step(fencs, h, enum)
            enum = self.evocab.numberize(ewords[i])
            copy_enum = self.evocab.numberize("<COPY>")
            # Add "<COPY>" if there is a match
            for j in range(len(fwords)):
                if ewords[i] == fwords[j]:
                    logprob += o[copy_enum] + cross_att[j]
            logprob += o[enum]
        return logprob
    ...
    def translate(self, fwords):
        ...
        # Maximum sentence length chosen to be 10
        # Categories aren't very long
        for i in range(10):
            o, h, cross_att = self.dec.step(fencs, h, enum)
            enum = torch.argmax(o).item()
            eword = self.evocab.denumberize(enum)
            if eword == '<COPY>':
                j = torch.argmax(cross_att).item()
                eword = fwords[j]
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        for epoch in range(10):
            ...
            print(f'[{epoch+1}] train_loss={train_loss} dev_loss={dev_loss} train_ppl={math.exp(train_loss/train_ewords)} dev_ppl={math.exp(dev_loss/dev_ewords)}', flush=True)
